...
class AuthService(
    ...
    fun registerFarmer(request: FarmerRegistrationDto): Result<Farmer> {
        return try {
            ...
            if (phoneNumber != null &&
                            userRepository.existsByPhoneNumberAndRole(
                                    phoneNumber,
                                    RoleType.FARMER.name
                            )
            ) {
                logger.warn(
                        "Registration failed: Phone number {} already registered as Farmer",
                        phoneNumber
                )
            ...
            val farmerRole =
                    roleRepository.findByName(RoleType.FARMER.name)
            ...
            val user =
                    if (existingUser != null) {
                        ...
                        val newUser =
                                createUser(
                                        request.user.copy(email = email)
                                ) // Update email to null if blank
            ...
            request.farmerProduces.forEach { produceDto ->
                try {
                    ...
                    logger.error(
                            "Failed to add produce for farmer {}: {}",
                            savedFarmer.id,
                            e.message,
                            e
                    )
            ...
            if (user.phoneNumber != null) {
                try {
                    ...
                    logger.warn(
                            "Failed to queue welcome SMS to {}: {}",
                            user.phoneNumber,
                            e.message
                    )
    ...
    fun registerBuyer(request: BuyerRegistrationDto): Result<Buyer> {
        return try {
            ...
            if (phoneNumber != null &&
                ...
                logger.warn(
                        "Registration failed: Phone number {} already registered as Buyer",
                        phoneNumber
                )
            ...
            val buyerRole =
                    roleRepository.findByName(RoleType.BUYER.name)
            ...
            if (request.preferredProduces.isNotEmpty()) {
                ...
                for (preferredProduce in request.preferredProduces) {
                    try {
                        ...
                        val existingProduce =
                                BSProduceRepository.findAll().find {
                        ...
                        val produceId =
                                if (existingProduce != null) {
                                    ...
                                    // Create new produce
                                    val newProduce =
                                            com.agriconnect.farmersportalapis.buyers.domain.common
                                                    .model.BSFarmProduce(
                                                    name = preferredProduce.name,
                                                    description =
                                                            preferredProduce.name, // Use name as
                                                    // description for now
                                                    farmingType = "GENERAL", // Default farming type
                                                    status =
                                                            com.agriconnect.farmersportalapis.buyers
                                                                    .domain.common.enums
                                                                    .FarmProduceStatus.ACTIVE
                                            )
                        ...
                        logger.error(
                                "Failed to process preferred produce '{}' for buyer {}: {}",
                                preferredProduce.name,
                                savedBuyer.id,
                                e.message,
                                e
                        )
                ...
                if (produceIds.isNotEmpty()) {
                    try {
                        buyerService.addProducesToBuyer(
                                com.agriconnect.farmersportalapis.buyers.application.dtos
                                        .AddProducesToBuyerDto(
                                                buyerId = savedBuyer.id!!,
                                                buyerProducesId = produceIds
                                        )
                        )
                        logger.info(
                                "Added {} preferred produces for buyer: {}",
                                produceIds.size,
                                savedBuyer.id
                        )
                    } catch (e: Exception) {
                        logger.error(
                                "Failed to add preferred produces for buyer {}: {}",
                                savedBuyer.id,
                                e.message,
                                e
                        )
                        throw PersistenceException(
                                "Failed to add preferred produces for buyer: ${e.message}",
                                e
                        )
                    }
                }
    ...
    fun registerExporter(request: ExporterRegistrationDto): Result<Exporter> {
        return try {
            ...
            if (phoneNumber != null &&
                ...
                logger.warn(
                        "Registration failed: Phone number {} already registered as Exporter",
                        phoneNumber
                )
            ...
            if (request.licenseId != null && exporterRepository.existsByLicenseId(request.licenseId)
                ...
                logger.warn(
                        "Registration failed: License ID {} already registered",
                        request.licenseId
                )
            ...
            val exporterRole =
                    roleRepository.findByName(RoleType.EXPORTER.name)
    ...
    fun registerAggregator(request: AggregatorRegistrationDto): Result<Aggregator> {
        return try {
            ...
            // Check for role-specific duplicates
            if (email != null &&
                            userRepository.existsByEmailAndRole(email, RoleType.AGGREGATOR.name)
            ) {
            ...
            if (phoneNumber != null &&
                ...
                logger.warn(
                        "Registration failed: Phone number {} already registered as Aggregator",
                        phoneNumber
                )
            ...
            val aggregatorRole =
                    roleRepository.findByName(RoleType.AGGREGATOR.name)
            ...
            val aggregator =
                    Aggregator(
                            ...
                            organizationType =
                                    request.organizationType?.let {
                                        try {
                                            AggregatorType.valueOf(it.uppercase())
                                        } catch (e: Exception) {
                            ...
                            storageCapacityTons =
                                    request.storageCapacityTons?.let { BigDecimal.valueOf(it) },
                            collectionRadiusKm =
                                    request.collectionRadiusKm?.let { BigDecimal.valueOf(it) },
            ...
            try {
                ...
                logger.warn(
                        "Failed to initiate async Hedera account creation for aggregator: {}",
                        e.message
                )
    ...
    fun registerProcessor(request: ProcessorRegistrationDto): Result<Processor> {
        return try {
            ...
            if (phoneNumber != null &&
                ...
                logger.warn(
                        "Registration failed: Phone number {} already registered as Processor",
                        phoneNumber
                )
            ...
            val processorRole =
                    roleRepository.findByName(RoleType.PROCESSOR.name)
            ...
            if (hederaAccountResult != null) {
                val credentials =
                        com.agriconnect.farmersportalapis.domain.hedera.HederaAccountCredentials(
                                userId = user.id!!,
                ...
                try {
                    ...
                    logger.warn(
                            "Failed to associate EUDR Certificate NFT with processor account: {}",
                            e.message
                    )
                }
            }
    ...
    fun registerImporter(request: ImporterRegistrationDto): Result<Importer> {
        return try {
            ...
            if (phoneNumber != null &&
                ...
                logger.warn(
                        "Registration failed: Phone number {} already registered as Importer",
                        phoneNumber
                )
            ...
            val importerRole =
                    roleRepository.findByName(RoleType.IMPORTER.name)
            ...
            if (hederaAccountResult != null) {
                ...
                try {
                    ...
                    logger.warn(
                            "Failed to associate EUDR Certificate NFT with importer account: {}",
                            e.message
                    )
                }
            }
    ...
    /**
     * Creates Hedera account asynchronously to avoid blocking registration Updates aggregator with
     * Hedera account details when creation completes
     */
    @org.springframework.scheduling.annotation.Async
    fun createHederaAccountAsync(userId: String, aggregatorId: String, organizationName: String) {
        ...
        try {
            ...
            logger.info(
                    "Hedera account created successfully: {} for aggregator ID: {}",
                    hederaAccountResult.accountId,
                    aggregatorId
            )
            ...
            val aggregator =
                    aggregatorRepository.findById(aggregatorId).orElseThrow {
            ...
            try {
                ...
                if (eudrCertificateNftId != null) {
                    ...
                    hederaAccountCredentialsRepository.save(credentials)
                    logger.info(
                            "EUDR Certificate NFT associated with aggregator Hedera account: {}",
                            hederaAccountResult.accountId
                    )
                }
            } catch (e: Exception) {
                logger.warn(
                        "Failed to associate EUDR Certificate NFT with aggregator account: {}",
                        e.message
                )
            }

            logger.info(
                    "Async Hedera account creation completed for aggregator ID: {}",
                    aggregatorId
            )
        } catch (e: Exception) {
            logger.error(
                    "Failed to create Hedera account asynchronously for aggregator ID: {}",
                    aggregatorId,
                    e
            )
    ...
    fun login(request: LoginDto): Result<LoginResponseDto> {
        ...
        logger.info(
                "[AuthService] Received login request: emailOrPhone={}, roleType={}",
        ...
        return try {
            ...
            logger.debug(
                    "Input validation passed: emailOrPhone={}, roleType={}",
                    request.emailOrPhone,
                    request.roleType
            )

            val user =
                    userRepository.findByEmail(request.emailOrPhone)
            ...
            logger.debug(
                    "[AuthService] Attempting authentication for emailOrPhone={}",
                    request.emailOrPhone
            )
            ...
            logger.debug(
                    "[AuthService] Authentication successful for emailOrPhone={}",
                    request.emailOrPhone
            )

            // Smart Role Detection (same as Google login)
            val userRoles =
                    user.roles
                            .filter {
                                it.name !in listOf("ADMIN", "SYSTEM_ADMIN", "ZONE_SUPERVISOR")
                            }
                            .map { it.name }

            if (userRoles.isEmpty()) {
                logger.warn("User {} has no valid roles for login", request.emailOrPhone)
                return ResultFactory.getFailResult("No valid roles found. Please contact support.")
            }

            // Determine which role to use
            val roleType: RoleType =
                    when {
                        // If role explicitly provided (traditional flow)
                        request.roleType != null -> {
                            // Verify user has the requested role
                            if (!userRoles.contains(request.roleType.name)) {
                                logger.warn(
                                ...
                                return ResultFactory.getFailResult(
                                        "User does not have ${request.roleType.name} role"
                                )
                            }
                            request.roleType
                        }
                        // If single role, use it automatically
                        userRoles.size == 1 -> {
                            logger.info(
                                    "User {} auto-login with role: {}",
                                    request.emailOrPhone,
                                    userRoles.first()
                            )
                            RoleType.valueOf(userRoles.first())
                        }
                        // Multiple roles - need user to select
                        else -> {
                            logger.info(
                                    "User {} has multiple roles: {}",
                                    request.emailOrPhone,
                                    userRoles.joinToString()
                            )
                            val roleOptions =
            ...
            val userDetailsWithRoleId =
                    userDetailsService.loadUserByUsernameAndRole(
                            request.emailOrPhone,
                            roleType.name
                    )
            val userDetails = userDetailsWithRoleId.userDetails
            logger.debug(
                    "[AuthService] UserDetails loaded: username={}, authorities={}",
            ...
            val hasRequestedRole =
                    userDetails.authorities.any { it.authority == "ROLE_${roleType.name}" }
            if (!hasRequestedRole) {
                logger.warn(
                        "[AuthService] User does not have requested role: email={}, role={}",
                        user.email,
                        roleType.name
                )
                return ResultFactory.getFailResult("User does not have ${roleType.name} role")
            }

            val roleSpecificData =
                    when (roleType) {
                        RoleType.FARMER ->
                                farmerRepository.findByUserProfile(user).get().let { f ->
                                    mapOf(
                                            "id" to (f.id ?: ""),
                        ...
                        RoleType.BUYER ->
                                buyerRepository.findByUserProfile(user).get().let { b ->
                                    BuyerLoginDto(
                                            id = b.id ?: "",
                        ...
                        RoleType.EXPORTER ->
                                exporterRepository.findByUserProfile(user.id).get().let { e ->
                                    ExporterLoginDto(
                                            id = e.id,
                        ...
                        RoleType.AGGREGATOR -> {
                            logger.debug(
                                    "[AuthService] Resolving Aggregator entity for userId={}",
                                    user.id
                            )
                        ...
                        RoleType.PROCESSOR -> {
                            logger.debug(
                                    "[AuthService] Resolving Processor entity for userId={}",
                                    user.id
                            )
                        ...
                        RoleType.IMPORTER -> {
                            logger.debug(
                                    "[AuthService] Resolving Importer entity for userId={}",
                                    user.id
                            )
                        ...
                        RoleType.ADMIN -> {
                            logger.debug(
                                    "[AuthService] Resolving Admin entity for userId={}",
                                    user.id
                            )
                        ...
                        RoleType.SYSTEM_ADMIN -> {
                            logger.debug(
                                    "[AuthService] Resolving System Admin entity for userId={}",
                                    user.id
                            )
                        ...
                        RoleType.ZONE_SUPERVISOR -> {
                            logger.debug(
                                    "[AuthService] Resolving Zone Supervisor entity for userId={}",
                                    user.id
                            )
            ...
            val permissions =
                    user.roles
                            .firstOrNull { role ->
                                role.name.uppercase().equals(request.roleType?.name)
                            }
                            ?.permissions
            val token =
            ...
            logger.debug(
                    "[AuthService] JWT token generated for emailOrPhone={}",
                    request.emailOrPhone
            )
            ...
            logger.info(
                    "[AuthService] Login successful for principal={}, roleType={}, elapsed={} ms",
                    request.emailOrPhone,
                    request.roleType,
                    elapsed
            )
            ...
            */
            ...
            logger.error(
                    "[AuthService] Authentication failed: Invalid credentials for emailOrPhone={}",
                    request.emailOrPhone,
                    e
            )
            return ResultFactory.getFailResult(
                    "Invalid " +
                            (if (request.emailOrPhone.contains("@")) "email" else "phone") +
                            " and password"
            )
        } catch (e: DisabledException) {
            logger.error(
                    "[AuthService] Authentication failed: Account disabled for emailOrPhone={}",
                    request.emailOrPhone,
                    e
            )
            ...
            logger.error(
                    "[AuthService] User not found for emailOrPhone={}: {}",
                    request.emailOrPhone,
                    e.message,
                    e
            )
            ...
            logger.error(
                    "[AuthService] Entity not found during login for emailOrPhone={}: {}",
                    request.emailOrPhone,
                    e.message,
                    e
            )
            ...
            logger.error(
                    "[AuthService] Unexpected error during login for emailOrPhone={}: {}",
                    request.emailOrPhone,
                    e.message,
                    e
            )
        ...
        return try {
            ...
            val user =
                    when {
                        ...
                        input.matches(Regex("^[0-9+\\-\\s()]{8,15}$")) ->
                                userRepository.findByPhoneNumber(input)
                                        ?: throw UsernameNotFoundException(
                                                "User with phone number $input not found"
                                        )
                        else ->
                                throw IllegalArgumentException(
            ...
            try {
                ...
                logger.error(
                        "Failed to delete existing tokens for user {}: {}",
                        input,
                        e.message,
                        e
                )
            ...
            try {
                ...
                logger.error(
                        "Failed to send password reset notification to {}: {}",
                        input,
                        e.message,
                        e
                )
                throw PersistenceException(
                        "Failed to send password reset notification: ${e.message}",
                        e
                )
            }
            ...
            ResultFactory.getSuccessResult(
                    true,
                    "Password reset OTP sent to your email or phone number"
            )
        } catch (e: PersistenceException) {
            logger.error(
                    "Database error during password reset request for {}: {}",
                    input,
                    e.message,
                    e
            )
            ...
            logger.error(
                    "Unexpected error during password reset request for {}: {}",
                    input,
                    e.message,
                    e
            )
    ...
    @Transactional(rollbackFor = [Exception::class])
    fun registerUser(request: UserRegistrationDto): Result<UserProfile> {
        return try {
            ...
            // Ensure at least one contact method is provided
            val hasEmail = request.email?.isNotBlank() == true
            val hasPhone = request.phoneNumber?.isNotBlank() == true
            ...
            if (email != null && userRepository.findByEmail(email) != null) {
                ...
                return ResultFactory.getFailResult("Email already registered")
            }
            if (phoneNumber != null && userRepository.findByPhoneNumber(phoneNumber) != null) {
                logger.warn("Registration failed: Phone number {} already registered", phoneNumber)
                return ResultFactory.getFailResult("Phone number already registered")
            }

            // Create new user
            val newUser =
                    createUser(
                            request.copy(email = email).copy(password = password)
                    ) // Update email to null if blank
            val savedUser = userRepository.save(newUser)

            logger.info(
                    "Generic user registered successfully: id={}, email={}",
                    savedUser.id,
                    savedUser.email
            )
            ...
            if (savedUser.email != null) {
                try {
                    ...
                    logger.info("Welcome email queued for user: {}", savedUser.email)
                } catch (e: Exception) {
                    logger.warn(
                            "Failed to queue welcome email to {}: {}",
                            savedUser.email,
                            e.message
                    )
                    // Don't fail registration if email queuing fails
            ...
            if (savedUser.phoneNumber != null) {
                try {
                    ...
                    logger.info("Welcome SMS queued for user: {}", savedUser.phoneNumber)
                } catch (e: Exception) {
                    logger.warn(
                            "Failed to queue welcome SMS to {}: {}",
                            savedUser.phoneNumber,
            ...
            ResultFactory.getSuccessResult(savedUser)
        } catch (e: PersistenceException) {
            logger.error("Database error during user registration: {}", e.message, e)
            throw e
        } catch (e: Exception) {
            logger.error("Unexpected error during user registration: {}", e.message, e)
            return ResultFactory.getFailResult("Failed to register user: ${e.message}")
        }
    }
    @Transactional(rollbackFor = [Exception::class])
    fun googleLogin(request: GoogleLoginDto): Result<LoginResponseDto> {
        val start = System.currentTimeMillis()
        logger.info("[AuthService] Received Google login request")
        ...
        return try {
            ...
            if (connection.responseCode != 200) {
                logger.warn("Google token verification failed: HTTP {}", connection.responseCode)
                return ResultFactory.getFailResult("Invalid Google token")
            }

            val responseStream = connection.inputStream.bufferedReader().use { it.readText() }
            val mapper = com.fasterxml.jackson.databind.ObjectMapper()
            val googleClaims = mapper.readTree(responseStream)

            val email = googleClaims.get("email").asText()
            val emailVerified = googleClaims.get("email_verified").asBoolean()

            if (!emailVerified) {
                return ResultFactory.getFailResult("Google email not verified")
            }
            ...
            if (user == null) {
                ...
                logger.info("[AuthService] Auto-registering new Google user: {}", email)

                val fullName = googleClaims.get("name")?.asText() ?: email.substringBefore("@")
                val givenName = googleClaims.get("given_name")?.asText()
                val familyName = googleClaims.get("family_name")?.asText()
                ...
                val newUser =
                        UserProfile(
                                fullName = fullName,
                                email = email,
                                phoneNumber = null, // Google doesn't provide phone number
                                passwordHash =
                ...
                logger.info("[AuthService] Created new user profile for Google user: {}", email)

                // Note: User has no specific roles yet - they'll need to complete registration
                // We'll handle this in the role detection logic below
            }
            ...
            // Get user's actual roles (exclude admin roles for regular portal login)
            val userRoles =
                    user.roles
                            .filter {
                                it.name !in listOf("ADMIN", "SYSTEM_ADMIN", "ZONE_SUPERVISOR")
            ...
            if (userRoles.isEmpty()) {
                logger.info("New Google user {} needs to complete registration", email)
                // Return special response for new users to complete registration
                return ResultFactory.getSuccessResult(
                        LoginResponseDto(
                                token = null,
                                roleSpecificData =
            ...
            val roleType: RoleType =
                    when {
                        // If role explicitly selected (second step of multi-role flow)
                        request.selectedRole != null -> {
                            try {
                                RoleType.valueOf(request.selectedRole.uppercase())
                            } catch (e: Exception) {
                                return ResultFactory.getFailResult("Invalid role selected")
                            }
                        }
                        // If single role, use it automatically
                        userRoles.size == 1 -> {
            ...
            // 4. Generate Token
            val userDetailsWithRoleId =
                    userDetailsService.loadUserByUsernameAndRole(email, roleType.name)
            val userDetails = userDetailsWithRoleId.userDetails

            val permissions =
                    user.roles.firstOrNull { role -> role.name == roleType.name }?.permissions
            val token =
                    jwtUtil.generateToken(
                            userDetails,
                            roleType.name,
            ...
            ResultFactory.getSuccessResult(loginResponse)
        } catch (e: Exception) {
            logger.error("Google login error: {}", e.message, e)
            ResultFactory.getFailResult("Google login failed: ${e.message}")
        }
    }

    @Transactional(rollbackFor = [Exception::class])
    fun assignRoles(request: AssignRolesDto): Result<Boolean> {
        ...
        request.roles.forEach { roleType ->
            val roleName = roleType.name
            val role =
                    roleRepository.findByName(roleName)
                            ?: throw EntityNotFoundException("Role $roleName not found")

            if (!user.roles.contains(role)) {
                user.roles.add(role)

                // Create corresponding entity profile if needed
                when (roleType) {
                    RoleType.FARMER -> {
    ...
    // Helper function to get description for role
    private fun getRoleDescription(roleName: String): String? {
        return when (roleName) {
            "FARMER" -> "Manage your farm, crops, and produce"
            "BUYER" -> "Browse and purchase agricultural products"
            "EXPORTER" -> "Export agricultural products internationally"
