...
import { throttleer as grecaptcha } from 'vue-infinite-loading/src/utils';
...
export default {
  ...
  data() {
    return {
      ...
      otp: '',
      isOtpGiven: false,
      userRole: this.$route.query.mode === 'exporter' ? 'farmer' : (this.$route.query.role || getCurrentUserRole()),
      user: {
        uid: '',
  ...
  methods: {
    ...
    async signUpUser() {
      try {
        // Determine role based on form data
        let role = 'buyer'; // default
        if (this.form.licenseId && this.form.licenseId.trim()) {
          role = 'exporter';
        } else if (this.form.farmName && this.form.farmName.trim()) {
          role = 'farmer';
        }

        // Set the userRole for later use
        this.userRole = role;
        ...
        if (role === 'farmer' || role === 'buyer') {
          const user = {
            ...
            [`${role === 'buyer' ? 'preferredProduces' : 'farmerProduces'}`]: [],
            location: {
          ...
          const saveResponse = await axios.post(`/${role}s-service/${role}`,
            user);
          if (saveResponse.data.success === true) {
            this.user = { ...this.user, ...saveResponse.data.data };
            this.$toast.success(`${role} profile set up`, 'success');
          } else {
